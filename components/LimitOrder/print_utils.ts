...
export class PrintUtils {
  ...
  public printTransaction(
    ...
    if (txReceipt.logs.length > 0) {
      ...
      for (const log of txReceipt.logs) {
        const decodedLog =
          this._web3Wrapper.abiDecoder.tryToDecodeLogOrNoop(log)
  ...
  public printOrderInfos(orderInfos: { [orderName: string]: OrderInfo }): void {
    ...
    _.forOwn(orderInfos, (value, key) =>
      data.push([key, OrderStatus[value.status]])
    )
  ...
  public async fetchAndPrintERC721OwnerAsync(
    ...
    const balances = [tokenSymbol]
    const token = new ERC721TokenContract(
      erc721TokenAddress,
      this._contractWrappers.getProvider()
    )
